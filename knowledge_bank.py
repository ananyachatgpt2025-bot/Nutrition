...
def _embed_texts(texts: List[str]) -> np.ndarray:
    ...
    inputs = [t[:6000] if t else "" for t in texts]
...
def kb_retrieve(db_path: str, query: str, top_k: int = 3) -> str:
    ...
    scores = X @ q
    # pull top indices mapping back to rows that had embeddings
    ...
    # Short banner style like the old "gold cases"
    return "\n\n".join(parts)
